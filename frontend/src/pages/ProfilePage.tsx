        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-5">Фото</h2>
              <div className="flex flex-col items-center gap-4 text-center">
                <div className="relative">
                  {photo ? (
                    ...
                    <div className="w-24 h-24 rounded-2xl bg-violet-100 flex items-center justify-center text-4xl font-black text-violet-600 border-2 border-violet-200">
                      {initials}
                    </div>
                  )}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">{isTutor ? "Репетитор" : "Ученик"}</p>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="subjects">{isTutor ? "Предметы (через запятую)" : "Предметы, которые изучаю"}</Label>
                  <Input id="subjects" value={subjects} onChange={(e) => setSubjects(e.target.value)}
                    placeholder={isTutor ? "Математика, Физика" : "Математика, Русский язык"} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="level">{isTutor ? "Уровень преподавания" : "Класс / уровень"}</Label>
            ...
            {/* Настройки оплаты — только репетитор */}
            {isTutor && (
              <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="wallet" value={wallet} onChange={(e) => setWallet(e.target.value)} placeholder="410011XXXXXXXXX" />
                    <p className="text-xs text-gray-400">
                      <a href="https://yoomoney.ru" target="_blank" rel="noreferrer" className="text-violet-600 hover:underline">yoomoney.ru</a> → «Номер счёта»
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="price">Цена занятия по умолчанию (₽)</Label>
                    <Input id="price" type="number" min="0" step="50" value={price}
                      onChange={(e) => setPrice(e.target.value)} placeholder="1500" />
                  </div>
                  <div className="space-y-1.5 sm:col-span-2">
                    <Label htmlFor="secret">Секрет для вебхука ЮMoney</Label>
                    <div className="relative">
                      <Input id="secret" type={showSecret ? "text" : "password"} value={secret}
                        onChange={(e) => setSecret(e.target.value)}
                        placeholder="Оставьте пустым, если не используете" className="pr-11" />
                ...
                <div className="flex gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700">
                  ...
                  <div>
                    ...
                    <p className="font-mono bg-blue-100 px-1.5 py-0.5 rounded mt-1 break-all">{window.location.origin}/yoomoney/webhook</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <button type="submit" disabled={saving || uploading}
              className={cn(
                ...
                saved ? "bg-emerald-500" : "bg-violet-600 hover:bg-violet-700 disabled:opacity-60"
              )}
            >
